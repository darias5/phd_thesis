...
abstract_sentence2 <- paste0("The economic value associated with this burden is estimated at about ",
               abstract4,
...
results_sentence1 <- paste0(
 ...
 abstract2,
 "\\% of global DALYs (Figure 2). Under all three methods, the burden of mental disorders (in DALYs) exhibited a country-income gradient, with mental disorders comprising ",
...
\emph{Daniel Arias, Shekhar Saxena, and Stéphane Verguet}
...
An earlier version of this paper was published by \emph{eClinicalMedicine} in Volume 54, Issue 101675 in December 2022.
...
The most recent estimates of morbidity and mortality due to mental disorders come from the Global Burden of Disease (\acr{GBD}) 2019 study.[@vosGlobalBurden3692020a] The \acr{GBD} study provides disease burden estimates using \acr{DALYs}, years of healthy life lost due to disability (\acr{YLDs}), and \acr{YLLs}, which are then aggregated within a hierarchical grouping scheme that classifies causes of disability and death at different levels of mutually exclusive and completely exhaustive categories. (Mental disorders are a Level 2 condition, nested under \acr{NCDs}; see Table S1.3.)

While \acr{GBD} remains the gold standard for global epidemiologic estimation, the nature of the \acr{GBD} scheme—in particular, the rationale for grouping certain conditions under mental disorders or not—has been the subject of debate in the literature.[@atunChallengesEstimatingTrue2016; @vigoEstimatingTrueGlobal2016; @whitefordChallengesEstimatingTrue2016] In particular, work by Vigo et al. (2016) published in \emph{The Lancet Psychiatry} argues for an expanded classification of mental disorders under the \acr{GBD} classification scheme to account for underestimation of the burden of mental disorders.[@vigoEstimatingTrueGlobal2016] The authors attribute this underestimation to five main causes: 1) the distinction drawn between mental and neurological diseases; 2) the categorization of self-harm and suicide under injuries; 3) the classification of all chronic pain and somatoform disorders under musculoskeletal disorders; 4) the exclusion of personality disorders; and 5) the exclusion of premature mortality due to mental disorders. Using data from the 2013 \acr{GBD} study, Vigo and colleagues re-allocated the entire burden of dementia, epilepsy, migraine, tension-type headache, and self-harm to mental disorders. In addition, a third of the burden of musculoskeletal disorders without anatomical correlate (i.e., somatoform disorders with prominent pain) was attributed to mental disorders.[@vigoEstimatingTrueGlobal2016] This reallocation attributed 13% of \acr{DALYs} to mental disorders, a 6 percentage point increase from the \acr{GBD} estimate of 7%.

In this paper, we attempt to revisit the estimation of the global burden of mental disorders and of its associated economic value. Our aim is to characterize potential underestimation of the burden of mental disorders and to quantify the economic value of this burden under different estimation approaches. Specifically, we expand on Vigo et al.[@vigoEstimatingTrueGlobal2016] by capturing premature mortality due to mental disorders using pooled risk ratios of mortality from a systematic review of mental disorders[@walkerMortalityMentalDisorders2015] to determine the \acr{PAF} of premature mortality. Inclusion of premature mortality through the \acr{PAF} presents a novel composite approach that can more broadly capture attributable morbidity and mortality. Using this approach on \acr{GBD} 2019 estimates, we then apply monetary values to \acr{DALYs} to reach estimates of the global economic value of the mental burden of disease using a \acr{VSL} approach similar to that of Bloom and colleagues,[@bloomGlobalEconomicBurden2011] which attempts to capture a population’s willingness to pay to reduce morbidity and mortality associated with illness. The \acr{VSL} approach—in contrast to alternatives such as cost of illness and value of lost output approaches—includes an economic valuation of mortality risk reductions in monetary terms, and thus enables comparison across sectors (beyond the sole health sector) which can motivate decision-making toward ameliorating welfare and societal mental health. Our findings suggest that both the epidemiological and economic burden of mental disorders could be larger than previously estimated, and that underestimation would be larger among regions where premature mortality due to mental disorders is greater.

## Methods

To estimate the economic burden of mental disorders, we first estimate the attributable mental burden of \acr{DALYs} under various estimation approaches using data from the 2019 \acr{GBD} study (available from the Global Health Data Exchange at https://ghdx.healthdata.org/gbd-2019). Second, we apply a monetary value to a DALY to yield an economic assessment associated with these burden estimates.

### Burden of mental disorders

In our analysis, we replicate the approach of Vigo et al. (2016) using \acr{GBD} 2019 estimates, applying a similar re-allocation formula to YLLs, YLDs, \acr{DALYs}, and deaths. Our approach, however, differs in some key respects.

First, we agree with Whiteford and colleagues in viewing the assigning of the entire burden of suicide and self-harm to mental disorders as an overestimate, and consequently do not reallocate all \acr{DALYs} due to suicide towards the mental health burden.[@whitefordChallengesEstimatingTrue2016] While it is empirically clear that mental disorders elevate the risk of death by suicide and that the majority of suicides appear to be due to mental disorders,[@bertoloteSuicidePsychiatricDiagnosis2002] we view assigning the entirety of this burden to mental disorders as overinclusive, which we avoid to favor a conservative estimation strategy.
...
Following Vigo and colleagues, we replicate reallocations in neurological and musculoskeletal conditions, and further include alcohol and mental use disorders, as these were previously classified under mental disorders within the \acr{GBD} classification.

This provides estimates of YLDs due to mental disorders. We then estimate the \acr{PAF} of mortality due to mental disorders, using \acr{GBD} prevalence estimates and relative risk estimates for natural-cause and unnatural-cause mortality generated from a systematic review and meta-analysis by Walker et al.[@walkerMortalityMentalDisorders2015] A comparison of our allocation approach with those of Vigo et al. and the original \acr{GBD} hierarchical allocation is shown in Table 1.1.
...
We separately estimate the \acr{PAF} for natural and unnatural causes of mortality. Using the calculated \acr{PAF} estimates, we estimate YLLs attributable to mental disorders by multiplying the \acr{PAF} by the national burden of mortality. For natural causes of death, we conservatively apply the \acr{PAF} against YLLs attributable to \acr{NCDs}. For unnatural causes of death, we apply the \acr{PAF} against YLLs due to self-harm and injuries. These YLLs are then combined with the YLDs calculated previously to provide \acr{DALYs}.

### Economic burden of disease

To estimate the economic cost associated with premature mortality and morbidity tied to mental illnesses, we assigned a monetary value to attributable \acr{DALYs}. \acr{VSL} approaches assign a monetary value to small reductions in mortality risks.[@robinsonValuingMortalityRisk2019] Drawing from these approaches, Jamison and colleagues have estimated monetary values of statistical life years,[@jamisonGlobalHealth20352013] which Khadka and colleagues recently adapted to quantify the economic value of changing mortality risk by cause of death in \acr{LMICs}.[@khadkaEconomicValueChanging2021] While \acr{VSL} approaches are not meant to assign monetary values to full life years or years lived with illness or disability,[@robinsonValuingMortalityRisk2019] the Copenhagen Consensus has previously implemented the use of \acr{GDP} per capita as a proxy for the monetary value of a DALY as a standard estimate.[@copenhagenconsensuscenterMethodology] Values of one and three times \acr{GDP} per capita have been suggested as proxies for the value of a DALY.[@bertramCostEffectivenessThresholds2016; @commissiononmacroeconomicsandhealthMacroeconomicsHealthInvesting2001] Estimates of $1,000 and $5,000 per DALY have been used, with the justification that these would be reasonable and convenient lower and upper values, particularly for low-income and lower-middle income countries.[@jamisonInfectiousDiseaseInjury2013; @jhaBenefitsCostsHealth2018]

Consistent with previous approaches, we use \acr{GDP} per capita (\acr{USD} 2019) for our base-case value of a DALY. \acr{GDP} inputs are reported in 2019 \acr{USD} and obtained from the World Bank’s World Development Indicators; for consistency with our epidemiological inputs, we convert to per capita values using \acr{GBD} population estimates.

### Sensitivity analyses

The primary focus of this paper concerns structural uncertainty in determining the burden of mental illness, resulting in the evaluation of three different estimation approaches. To address parameter uncertainty within each approach, we apply a three-way sensitivity analysis. First, following a simple intuitive approach, we incorporate the upper and lower \acr{UIs} provided by \acr{GBD} 2019 for \acr{YLLs}, \acr{YLDs}, and \acr{DALYs} to account for parameter uncertainty. Second, we use the upper and lower values of prevalence estimates and of the 95% \acr{CIs} of the pooled relative risk of all-cause mortality from Walker and colleagues in our composite approach.[@walkerMortalityMentalDisorders2015] Third, our lower bound estimates are set to reallocate one sixth of the burden of musculoskeletal disorders proposed by Vigo and colleagues,[@vigoEstimatingTrueGlobal2016] while our upper bound estimates are set to reallocate one half of this burden.

In addition to our base-case economic valuation, we further report our \acr{VSL} estimates using three times \acr{GDP} per capita as the value of a DALY. 
...
The \acr{IRB} of the Harvard T.H. Chan School of Public Health determined that this study was not human subjects research and that additional review was not required (protocol number: IRB20-1946, determined on November 13, 2020).
...
Rates of \acr{DALYs} and deaths attributable to mental disorders under the different estimation approaches are presented by \acr{GBD} region (Figure \ref{figure_gbd_adjusted}). Geographically, the composite approach allocated a large portion of \acr{DALYs} (to mental disorders) in Eastern Europe, North and Latin America, and sub-Saharan Africa. This is largely driven by the inclusion of premature mortality in the composite approach. Estimates of DALYs by country income group and \acr{GBD} region are reported in Table 1.2. (Estimates of deaths are reported in Table S1.4-1.6.)

Under the three approaches, we calculated the economic value of mental disorder losses (Table 1.3). `r results_sentence2`. Further adjusting for purchasing power parity, the global value of mental illness losses would exceed 7.2 trillion international dollars in 2019 (Table S1.7). (Alternative valuations using values of $1,000 and $5,000, as well as \acr{PPP}-adjusted \acr{GDP} per capita, are reported in Table S1.8 of Appendix 1.)
...
```{r figure_dalys_nation, include = TRUE, echo = FALSE, warnings = FALSE, fig.scap="Disability-adjusted life years (DALYs) attributable to mental disorders in 2019, under the composite estimation approach, as a share of all DALYs.", out.extra='', fig.cap=paste0("\\label{figure_dalys_nation}Disability-adjusted life years (DALYs) attributable to mental disorders in 2019, under the composite estimation approach, as a share of all DALYs. ATG: Antigua and Barbuda. FSM: Federated States of Micronesia. LCA: Saint Lucia. TLS: East Timor. TTO: Trinidad and Tobago. VCT: Saint Vincent and the Grenadines."), fig.width = 13.7, fig.height = 7.5, fig.align='center', out.width = '98%'}
...
estimates_values <- c(1,2,4)

for (z in estimates_values) {
  

data_temporary <- data_rev_map %>% 
 filter(measure_id == 2) %>% 
 filter(estimate_id == z) %>%
 st_drop_geometry() %>%
...
ggsave(filename = paste0("Figure_2_estimate_",z,".jpg"), 
    plot = final_figure, 
    width = 13.33,
    height = 6.3,
    bg = "white")
...
ggsave(filename = paste0("Figure_3_DALY.png"), 
    plot = dalys, 
    width = 12,
    height = 6.3,
    bg = "white")

ggsave(filename = paste0("Figure_3_YLD.png"), 
    plot = ylds, 
    width = 12,
    height = 6.3,
    bg = "white")

ggsave(filename = paste0("Figure_3_YLL.png"), 
    plot = ylls, 
    width = 12,
    height = 6.3,
    bg = "white")

ggsave(filename = paste0("Figure_3_deaths.png"), 
    plot = deaths, 
    width = 12,
    height = 6.3,
    bg = "white")
...
```{r figure_cost_by_region, fig.width = 8, fig.height = 6, fig.align='center', out.width = '95%', fig.scap="Economic burden of mental disorders, as a percent of GDP.", out.extra='', fig.cap="\\label{figure_cost_by_region}Economic burden of mental disorders, as a percent of GDP. The economic value is determined by using GDP per capita (USD 2019) as the value of a DALY. Values are aggregated by GBD region. GDP: gross domestic product. USD: United States dollars. DALY: disability-adjusted life year. GBD: Global Burden of Disease.", echo = FALSE, include = TRUE}
...
ggsave(filename = paste0("Figure_4.png"), 
    plot = last_plot(), 
    width = 8,
    height = 6,
    bg = "white")
...
\acr{GBD} estimates are available for download from the Global Health Data Exchange and are available freely for non-commercial users under the Open Data Commons Attribution License (https://ghdx.healthdata.org/gbd-2019). All codes used for the analysis in this article are available on GitHub (https://github.com/darias5/gmh_econ).
