...
The health facility data used in our study were obtained from Madagascar's *Gestion du Système d'Information Sanitaire* (\acr{GESIS}), a Microsoft Access electronic health management information systems database. 

While primary, district, and regional referral facilities in Madagascar all reported monthly visits related to mental disorders in \acr{GESIS}, case definitions varied across facilities of different levels. Disaggregated data by specific condition (e.g., depression, schizophrenia, etc.) were only reported among regional referral hospitals and university hospitals; for all other facilities, visits by type of mental heath complaint were reported in aggregate (e.g., "Neuro-psychic diseases" for district hospitals).

Disaggregated data were available by age; among adults, however, disaggregated data for adults ages 25 to 59 and adults 60 years and older were only reported by primary health facilities after 2015. For all other years and all other facilities, these age categories were not separately reported. Similarly, data by sex were only reported for primary health facilities after 2015. Referrals were reported by primary health facilities and district hospitals.

A summary of \acr{GESIS} data availability is provided in Table S2.1.
...
After compiling the \acr{GESIS} data, we translated the case definitions from French to English. The translations are provided in Table S2.2.

\newpage

\addcontentsline{lot}{table}{Table S2.2: Mental health disorders reported in GESIS, in French and English.}

```{r table_s_2_2, echo = FALSE, include = TRUE, out.width = '97%'}

knitr::include_graphics("table_s_2_2.png")
...
data %>% filter(sex == "both" & subgroup == "total") %>% 
    ...
    # ungroup() %>%
    # dplyr::select(clinic_name, disease, year, ) %>%
    # unique() %>%
    # mutate(n = 1) %>% 
    # group_by(year, disease) %>%  
    mutate(sum = sum(count)) %>%  
    dplyr::select(sum, year, disease) %>% 
    ungroup() %>%
    unique() %>% 
...
n_chu <- chu_data %>% dplyr::select(disease) %>% unique() %>% nrow
...
We then conducted exploratory data analysis to gauge data availability by condition, year, and facility level, which are reported in Tables S2.3-5. These tables show yearly cumulative visits for mental and neurological disorders, by facility level.

Among primary health facilities, we observed that the case definition for mental disorders changed during 2015, alongside the changes in \acr{GESIS} that enabled disaggregated reporting by sex and greater disaggregation for data by age.

Among district hospitals, reporting of mental disorders was consistent between 2010 and 2020.

Reporting of mental disorders was most detailed among regional referral hospitals (CHRRs) and university hospitals (CHUs), with visit counts reported for `r  prettyNum(n_chu, big.mark = ",", scientific = FALSE)` unique conditions.

\begin{spacing}{1}

\addcontentsline{lot}{table}{Table S2.3: Yearly cumulative visits for mental and neurological disorders reported at primary facilities.}


```{r  reporting_tables1a, warning = FALSE, message = FALSE, echo = FALSE, include = TRUE, out.width = '97%'}

knitr::include_graphics("table_s_2_3.png")

# 
# reporting_table(csb_data, "Table S2.3: Yearly cumulative visits for mental and neurological disorders reported at primary facilities")  %>%
#    gtsave("table_s_2_3.docx")

```

\addcontentsline{lot}{table}{Table S2.4:  Yearly cumulative visits for mental and neurological disorders reported at district hospitals.}

```{r  reporting_tables1b, warning = FALSE, message = FALSE, echo = FALSE, include = TRUE, out.width = '97%'}

knitr::include_graphics("table_s_2_4.png")

# reporting_table(chd_data, "Table S2.4: Yearly cumulative visits for mental and neurological disorders reported at district hospitals") %>%
#    gtsave("table_s_2_4.docx")

```

\newpage

\addcontentsline{lot}{table}{Table S2.5: Yearly cumulative visits for mental and neurological disorders reported at regional hospitals.}

```{r  reporting_tables2, warning = FALSE, message = FALSE, echo = FALSE, include = TRUE, out.width = '97%'}

knitr::include_graphics("table_s_2_5a.png")
knitr::include_graphics("table_s_2_5b.png")


# reporting_table(chu_data, "Table S2.5: Yearly cumulative visits for mental and neurological disorders reported at regional hospitals") %>%
#    gtsave("table_s_2_5.docx")
...
data_mh_temp <- data_mh %>%
   dplyr::select(!c(disease, count)) %>%
   unique() %>%
   pivot_longer('Any mental or neurological disorders', names_to = "disease", values_to = "count")


data_mh <- data_mh %>%
  dplyr::select(!c('Any mental or neurological disorders')) %>%
  rbind(., data_mh_temp)

rm(data_mh_temp,csb_data, chd_data, chu_data, mh_english_disease)

data_mh %>% filter(sex == "both") %>% dplyr::select(clinic_name, district_name, region_name, commune_name) %>% unique() %>% nrow()
...
To address heterogeneity in reporting across primary, district, and regional facilities, our analysis constructed a single, summary outcome measure: any mental or neurological disorders reported in a given month by a given facility, irrespective of age and sex. This outcome measure was constructed by grouping our count data by facility and month-year, and then summing the incidence of mental and neurological disorders reported in \acr{GESIS} within groups, preventing double counting and the inclusion of referrals in the summary measure.
...
data_mh %>%
  ...
  labs(x = "Date", y = "Monthly count of visits") +
...
<!--   dplyr::select(date, region_name, district_name, commune_name, clinic_name_clean, clinic_type, reporting) %>% -->
<!--   unique() -->


<!-- duplicate_name_facilities <- data_facility_reporting %>% -->
<!--   dplyr::select(region_name, district_name, commune_name, clinic_name_clean, clinic_type) %>% -->
<!--   unique() %>% -->
...
<!--   ungroup() %>% -->
<!--   unique() %>% -->
<!--   dplyr::select(clinic_name_clean, clinic_type) %>% -->
...
## variable_descrip <- data_raw %>% dplyr::select(description) %>% unique()
## variable_name <- data_raw %>% dplyr::select(variable) %>% unique()
## variable_tabname <- data_raw %>% dplyr::select(table_name) %>% unique()
## variable_medco <- data_raw %>% dplyr::select(medical_code) %>% unique()
## 
...
## facilities_clinic_codes <- data_raw %>% dplyr::select(clinic_code) %>% unique()
## facilities_district_codes <- data_raw %>% dplyr::select(district_code) %>% unique()
## 
...
##                                  header=TRUE) %>% 
##   dplyr::select(district_code, clinic_code, name) %>%
...
##                                 header=TRUE) %>% 
##   dplyr::select(district_code, district_type, name) %>%
...
##                                    header=TRUE) %>%
##   dplyr::select(Nom.complet.FS.1, latitude, longitude) %>% 
...
##   dplyr::select(district_name, ADM2_PCODE)
## 
...
##   dplyr::select(district_code, district_name, ADM2_PCODE) %>%
##   unique()
## 
...
## facilities_map_clean <- facilities_map %>% dplyr::select(clinic_code:clinic_name) %>%
##   left_join(., district_crosswalk, by = "district_code") %>%
##   dplyr::select(!c(district_name)) %>%
...
## map_clean %>%
##   dplyr::select(ADM2_PCODE, district_name, district_code, geometry) %>%
